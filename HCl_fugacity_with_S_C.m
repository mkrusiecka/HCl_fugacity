...
%you have to set P, T, composition anf fO2 (fO2 is set starting line 110 -
%you choose a buffer (FMQ, NNO, CCO) and set offset, buffer choice is line
%116 and offset is line 117; water fugacity model can be specified line 72
%- either Burnham 1979 or Moore 1998
function HCl_fugacity_with_S_C()
    clear; clc;
    ...
    logfHCl_RW = 1.717 +log10(Cl_mes)+(-4784.*XCaO + 2975.*XSiO2 - 2705.*XFeO - 541.*XMgO + 2661.*XK2O - 4744.*XAl2O3 - 3635.*XNa2O + 766.*PG)./T;
    ...
    logC=1.492+(4331.*XCaO-3508.*XSiO2+2440.*XFeO-3921.*XK2O-741.*PG)./T; %log CCl Rusiecka & Wood 2025
    CCl_calc = 10.^logC;

%% USER SELECTION: Choose H2O fugacity model
H2O_model = 'Moore';   % Options: 'Moore' or 'Burnham'
switch H2O_model
    case 'Moore' %Mooore 1998 water activity model
        CatSum_an=SiO2_mol+TiO2_mol+(bulkComp(3,:)./(101.96))'+FeO_mol+MgO_mol+CaO_mol+(bulkComp(12,:)./(61.97))'+(bulkComp(13,:)./(94.2))'+MnO_mol;
        ...
        a_M=2565; %Moore et al. 1998 parameters
        bAl=-1.997;
        bFe=-0.9275;
        bNa=2.736;
        c=1.171;
        d=-14.21;
        B_M=(bAl.*XAl2O3_an.*(P./T)+bFe.*XFeO_an.*(P./T)+bNa.*XNa2O_an.*(P./T));
        lnfH2O_M=(2.*log(XH2O_an)-a_M./T-B_M-d)./c; %Mooore et al. 1998
        fH2O=exp(lnfH2O_M);
    case 'Burnham' %Burnham 1979 water activity model 
    Po = 1.01325; % bar
    ...
    kw = exp(lnkw);
   % --- Modify water activity (aH2O) calculation to use 8-oxygen basis mole fraction X_H2O_O instead of H2O_mol./CatSum ---
aH2O = zeros(size(T));
for idx = 1:length(T)
    if X_H2O_O(idx) >= 0.5
    ...
    otherwise
        error('Unknown H2O_model. Choose ''Moore'' or ''Burnham''.');
end  
...
buffer_offset = 0.0;             % numeric Δlog10 fO2 relative to the buffer
...
fO2    = 10.^logfO2;                   % fO2 in bar
    ...
    %% Kress and Carmichael 1991 (necessary for the S speciation from B&W 2022&2023
...
for i = 1:n
    ...
    % SO4 (wt%)  = 10^logCS6 * fS2^(1/2) * fO2^(+3/2)   
    massbal = @(logfS2) ...
...
% CH4 from graphite: C + 2H2 = CH4
logK_CH4_graphite = 4436.6 ./ T - 5.5907;
K_CH4_graphite = 10.^logK_CH4_graphite;
% CH4 from CO + 3H2 = CH4 + H2O
logK_CH4_CO = 4795.0 ./ T - 4.170;
K_CH4_CO = 10.^logK_CH4_CO;
...
for i = 1:length(T)
    ...
    outer_max = 150; outer_tol = 5e-3; damp = 0.5;
...
for it = 1:outer_max
    ...
    % Use the ratio when undersaturated; direct Kf when carbon-saturated
...
% --- Aux with your K(T) and capacity
aux = struct();
aux.KH2O       = KH2O(:);
aux.KHCl       = KHCl(:);
...
aux.opts       = struct('tol',1e-10,'maxIter',400,'verbose',false); % MRK solver opts

% --- Critical constants and kIJ used for MRK ---
crit = struct();
crit.Tc = [324.7; 647.3; 416.9;  33.2; 430.8; 373.2; 304.13; 132.9];
crit.Pc = [ 83.1; 220.64; 76.1; 12.97;  78.8;  89.6;  73.77; 34.94];
kIJ  = zeros(8);  
...
%% Tables and Excel file
Out = table(temperature, P, fH2_eos, fCl2_eos, fHCl_eos, fH2O_eos, Cl_mes, mole_fractions(:,1), (bulkComp(15,:))', 'VariableNames', {'T_C','P_bar','fH2_bar','fCl2_bar','fHCl_bar','fH2O_bar', 'Cl (wt. %)', 'X HCl', 'H_2O (wt. %)'});
writetable(Out, 'Fugacities.csv');
%% Hand-off (ONLY the fields you asked for)
% Mole fractions in this order: {'HCl','H2O','Cl2','H2','SO2','H2S','CO2','CO'}
X_HCl = mole_fractions(:,1);
X_H2O = mole_fractions(:,2);
X_Cl2 = mole_fractions(:,3);

% Minimal CSV: fO2 + X's (preferred for prescribed modes)
handoff_min = table(fO2(:), X_H2O(:), X_Cl2(:), X_HCl(:), ...
    'VariableNames', {'fO2_bar','X_H2O','X_Cl2','X_HCl'});
writetable(handoff_min, 'handoff_min.csv');
end
...
% ------------------ Per-oxygen atomic fractions for fwone 
...
Fco2 = [1.9244e-5, -9.0212e4, 1.1149e3, -43.0815, -7.0937];
    ...
    function [wtCO3, wtCO2, total_wt] = dissolvedCO2_at_f(fco2_bar)
        ...
        % Convert mole fractions to wt% using "fwone" 
...
% Water–gas check for UNDDERSATURATED rows:
...
thr_rxn = 0.04;   % 0.02 in log10 ~ 4.7% multiplicative error
